...
C ---- 2D Plane Stress (Voigt: 11,22,12) ----
     ...
     &                  +(ConstD*II1/(one-two*NU))+(ConstC*MaxPS/E)
            ...
            if (kappa .gt. kappa_o) then
			   ...
			   if (kappa .le. (Betaa+kappa_o)) then
					damage = One - (kappa_o / kappa)*(One-0.8*Betaa*(kappa-kappa_o))
			   else
					damage = One - (kappa_o / kappa)*(0.25-0.05*Betaa*(kappa-kappa_o))
			   end if
			   if (damage .gt. one) then
					damage = One
			   end if				   
